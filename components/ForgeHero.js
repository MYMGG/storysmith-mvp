...
export default function ForgeHero({
  ...
  const handleForgeHeroCompletion = () => {
    // Build the CharacterBlock in canonical format
    const characterBlock = {
      hero_name: heroDetails.name || 'Unknown Hero',
      hero_description: `A ${heroDetails.age || '7'}-year-old ${heroDetails.gender || 'child'} with ${heroDetails.traits || 'brave and kind'} traits.`,
      hero_image_url: heroImageUrl || null,
      hero_image_prompt: constructHeroPrompt(heroDetails),
      traits: {
        wardrobe: heroDetails.wardrobe,
        signatureItem: heroDetails.signatureItem,
      },
    };
    ...
    setSharedResponse("🎉 Your hero is forged! Download your Hero Bundle to continue the adventure! 🎉");
    setCurrentForgeHeroStep(4); // Show completion screen with export button
  };

  /**
   * Handles exporting the hero bundle as a downloadable JSON file.
   */
  const handleExportBundle = () => {
    try {
      // Build complete storyState for export
      const exportState = {
        ...storyState,
        story_content: {
      ...
      const bundle = exportBundle(exportState, 'Part1');

      // Trigger download
      const blob = new Blob([bundle.jsonString], { type: bundle.mime });
      ...
      setSharedResponse("✨ Hero Bundle downloaded! You may now continue to Act II or download again. ✨");
    } catch (err) {
      console.error('[ForgeHero] Export failed:', err);
      setSharedResponse(`Export failed: ${err.message}`);
    }
  };

  /**
   * Proceeds to Act II after export is complete.
   */
  const handleProceedToActII = () => {
    setActiveTab(1);
  };
